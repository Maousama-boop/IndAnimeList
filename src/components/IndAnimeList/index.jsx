            ...
            {api.data?.map((anime, index) => {
                return (
                    <Link href={`/anime/${anime.mal_id}`}className="cursor-pointer text-color-primary hover:text-color-accent transition-all" key={index}>
                        <Image src={anime.images.webp.image_url} width={350} height={350} className="w-full h-full max-h-64 md:max-h-72 anime"/>
                        <div className="overlay">
                          <h3 className="font-bold md:text-x text-md text-center title-anime">{anime.title}</h3>
                          <p className="md:text-x text-md">{anime.data.score}</p>
                        </div>
                    </Link>
                )    
            })}
